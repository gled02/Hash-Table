...
module MakeCoupleHashMap:CoupleHashMap with type clef = int with type valeur = string =
  struct
    type clef = int
  	let clef_comp = (-)
  	type valeur = string
...
module type TableDeHachage =
  sig
  	type hashValToKey
  	type valeur
  	val mapVide : hashValToKey
  	val estDans : hashValToKey * valeur -> bool
  	val insertion : hashValToKey * valeur -> hashValToKey
  	val suppression : hashValToKey * valeur -> hashValToKey
		val union : hashValToKey -> hashValToKey -> hashValToKey
		val intersection : hashValToKey -> hashValToKey -> hashValToKey
		val difference : hashValToKey -> hashValToKey -> hashValToKey
		val differenceSym : hashValToKey -> hashValToKey -> hashValToKey
		val appliquer_sur : (valeur -> valeur -> valeur) -> hashValToKey -> valeur -> valeur
  end
  
module MakeTableDeHachage (C : CoupleHashMap) : TableDeHachage with type valeur = C.valeur =
 	struct
		(*Types -------------------------------------------------- *)
  	...
  	type 'a arbre = Vide | VideNoir | Noeud of ('a * couleur * 'a arbre * 'a arbre)
		...
		let clef_comp = C.clef_comp
		let val_comp = C.val_comp
		let hash = C.hash
  	...
  	let rec estDansArbreValeur (a,s) =
  		match a with
  			| Vide | VideNoir -> false
  			| Noeud(x,_,_,_) when (val_comp x s) = 0 -> true
  			| Noeud(x,_,_,d) when (val_comp x s) < 0 -> estDansArbreValeur (d,s)
  			| Noeud(x,_,g,_) -> estDansArbreValeur (g,s);;
  
  	let rec estDans (a,s) =
  		match a with
  			| Vide | VideNoir -> false
  			| Noeud(x,_,_,_) when (clef_comp (fst x) (hash s)) = 0 && estDansArbreValeur((snd x),s) -> true
  			| Noeud(x,_,_,_) when (clef_comp (fst x) (hash s)) = 0 && not(estDansArbreValeur ((snd x),s)) -> false
    ...
    let insertion (a,v) =
  		let rec aux = function
				...
				| Noeud(e,c,ag,ad) as ab when not(estDans (ab,v)) && (clef_comp (hash v) (fst e)) = 0
						-> equilibrer (Noeud(((fst e),insererArbreValeur v (snd e)),c,ag,ad))
	  ...
	  let probleme = function
		...
		let rec supprimerMaxArbreValeur = function
			...
			| Noeud(r,c,g,d) -> let (m, d2) = (supprimerMaxArbreValeur d) in (m,equilibrerSupp (Noeud(r,c,g,d2)));;
		...
		let supprimerArbreValeur x a =
			let rec aux = function
				...
				| Noeud(v,c,g,d) -> let (m,g2) =  supprimerMaxArbreValeur g in equilibrerSupp (Noeud(m,c,g2,d))
			in stabiliser (aux a);;
		...
		let suppression (a,x) =
			let rec aux = function
				...
				| Noeud((n,s),c,Vide,Vide) as ab when estDans (ab,x) -> aux (Noeud((n,(supprimerArbreValeur x s)),c,Vide,Vide))
				| Noeud((n,s),Noir,Vide,Noeud(y,Rouge,Vide,Vide)) as ab when estDans (ab,x) -> equilibrerSupp (Noeud((n,(supprimerArbreValeur x s)),Noir,Vide,Noeud(y,Noir,Vide,Vide)))
				| Noeud((n,Noeud(_,_,Vide,Vide)),c,g,d) as ab when estDans (ab,x)
						-> let (m,g2) = supprimerMaxHash g in equilibrerSupp (Noeud(m,c,g2,d))
				| Noeud((n,s),c,g,d)
						-> equilibrerSupp (Noeud((n,(supprimerArbreValeur x s)),c,g,d))
			in stabiliser (aux a);;
...
module Test = MakeTableDeHachage (MakeCoupleHashMap);;
open Test;;
